...
```bash
# 交互模式：先列出所有对话，输入序号查看
python cursor_agent_transcript_viewer.py

# 仅列出所有对话（含对话 ID 与首句标题）
python cursor_agent_transcript_viewer.py list

# 查看第 N 条对话
python cursor_agent_transcript_viewer.py view 3

# 按文件路径查看
python cursor_agent_transcript_viewer.py view "C:\path\to\xxx.jsonl"

# 将第 N 条的对话 ID 复制到剪贴板（Windows）
python cursor_agent_transcript_viewer.py copy-id 2
```
