...
| BN 任务 `HTTP 412 Precondition Failed`(`Unable to download JSON metadata`) | bilibili 对 **BN 容器 yt-dlp 下载器**的风控(非本服务、非 SP-5b 代码;整个 B 站垂直 SP-4b 同样受影响)。推 cookie 给 BN(`POST /api/update_downloader_cookie`)实测**不**解;需在 BN/引擎侧处理(更新 BN 的 yt-dlp、换出口 IP、或补 wbi/反检测 headers)。受影响视频不入 seen、水位按 §5 守在失败值下方,BN 恢复后下轮自动重试落盘 |
