...
| 启动即报 `ImportError: Using SOCKS proxy ... socksio` | **冻结引擎的 `BiliNoteClient` 建 httpx.Client 未设 `trust_env=False`**,继承了宿主 `ALL_PROXY`(SOCKS)→ 连本地 BN 也想走代理。修法:运行环境**清空 `ALL_PROXY`/`HTTP(S)_PROXY`**(本服务自身的 SP-1/B站请求已 `trust_env=False`),或装 `httpx[socks]` 并设 `NO_PROXY=127.0.0.1,localhost`。docker `network_mode: host` 部署须确保容器内无 SOCKS 代理 env |
| BN 任务 `HTTP 412 Precondition Failed`(`Unable to download JSON metadata`) | **根因已实证(BN-412,2026-06-11)**:bilibili **playurl 接口对匿名(无 cookie)请求一律 412**——与出口 IP、yt-dlp 版本无关(容器内单变量矩阵:yt-dlp 2025.03.31/2026.06.09 无 cookie 均 412、带 cookie 均 200;同 IP 浏览器 UA 调 view API 200)。「推 cookie 不解」是**假阴性**:BN 的 `BilibiliDownloader` 是**导入时单例**,cookie 只在进程启动时读一次(`constant.py` `'bilibili': BilibiliDownloader()`)→ **修法 SOP:① `POST /api/update_downloader_cookie` 推含 SESSDATA 的 cookie;② 确认容器内 `/app/backend/config/downloader.json` 已存;③ `docker restart jarvankb-bilinote` 让单例重读;④ 容器内 `yt-dlp --cookies <file> --simulate --print title <url>` 验证**。该 SOP 已自动化:宿主 cron 每 10 分钟跑 `Engine/bilibili/deploy/bilinote/bn-cookie-sync.py`(SESSDATA 变化才推+重启,详见 deploy README)。受影响视频不入 seen、水位按 §5 守在失败值下方,BN 恢复后下轮自动重试落盘 |
