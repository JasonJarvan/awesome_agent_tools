...
- [ ] **Step 1: Run all three suites green — ONE INVOCATION PER PACKAGE**

Run each package SEPARATELY (a single combined `pytest` invocation hits a basename clash on the two
`test_classify.py` files — `Engine/common/tests/` and `Skill/crawl/zhihu-crawl/tests/` — because neither
test dir has `__init__.py`; that is a pytest collection artifact, not a code issue):

```bash
PYTHONPATH="$PYP" python -m pytest Engine/common/tests/ -q
PYTHONPATH="$PYP" python -m pytest Skill/crawl/zhihu-crawl/tests/ -q
PYTHONPATH="$PYP" python -m pytest Service/crawl/zhihu-watcher/tests/ -q
```
Expected: ALL PASS — jarvankb_common classify (22), SP-3 regression (27), watcher full suite (78) = 127 total.
