...
**下载器 cookie 在进程启动时只读一次（BN-412 实证 2026-06-11）**：BN 的 `BilibiliDownloader` 是模块导入时
构造的单例，`POST /api/update_downloader_cookie` 只写存储（`/app/backend/config/downloader.json`）、
**不会进入已存活的进程** —— 推完 cookie 必须 `docker restart jarvankb-bilinote` 才生效。背景：bilibili 的
playurl 接口对匿名（无 cookie）请求一律 `HTTP 412`（与出口 IP、yt-dlp 版本无关），所以**没有有效 SESSDATA
