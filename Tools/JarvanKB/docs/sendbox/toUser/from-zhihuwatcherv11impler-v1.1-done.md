...
3. **403 失败退避(轻量)** —— 连续失败 N 次进入时间窗 cooldown,成功即清零。专栏永久 403 的项不再每轮重试。
...
- **专栏全文抓取**:编排层已查明专栏 `/api/v4/articles` 需 x-zse-96 签名,你已选**不加签名器(UN-028)**→ 专栏全文永久抓不到;v1.1 用 backoff 兜住浪费。(注:实测**部分**专栏页导航 GET 其实能成,403 只是子集。)
