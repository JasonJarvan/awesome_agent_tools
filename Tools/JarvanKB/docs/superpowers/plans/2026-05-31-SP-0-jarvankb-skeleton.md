...
- **CWD throughout this plan**: `~/Codes/awesome_agent_tools/Tools/JarvanKB/`
- **If user has already renamed `Tools/JarvanKB/` → `Tools/JarvanKB/`** in a separate session: substitute `Tools/JarvanKB/` for every `Tools/JarvanKB/` path below. The rename is **independent** of this plan; this plan works either way.
...
- The repo physical rename `Tools/JarvanKB/` → `Tools/JarvanKB/` (user does in another session)
...
```bash
cd ~/Codes/awesome_agent_tools/Tools/JarvanKB/
...
git add Tools/JarvanKB/Engine Tools/JarvanKB/Skill Tools/JarvanKB/Service
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/Engine/common Tools/JarvanKB/config
...
```bash
cd ~/Codes/awesome_agent_tools/
git add -A Tools/JarvanKB/openspec Tools/JarvanKB/docs/openspec
...
```bash
cd ~/Codes/awesome_agent_tools/
git add -A Tools/JarvanKB/.claude
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/CLAUDE.md
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/docs/HarnessStack/longterm.md
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/docs/HarnessStack/README.md Tools/JarvanKB/docs/HarnessStack/_toUser/README.md
...
- Physical rename `Tools/JarvanKB/` → `Tools/JarvanKB/` performed in a separate session (tracked as Dashboard UN-005)
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/docs/RepoMem/persist/
...
|---|---|---|---|---|---|---|
| UN-005 | B | 在独立 session 中执行物理改名：`mv Tools/JarvanKB Tools/JarvanKB`，确认 git mv 路径无丢失，commit | `docs/superpowers/specs/2026-05-31-SP-0-jarvankb-skeleton-design.md` §2 | SP-1 brainstorming 启动（路径稳定后） | 2026-05-31 | open |
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/docs/Dashboard/index.md
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/README.md
...
```bash
cd ~/Codes/awesome_agent_tools/
git status --short Tools/JarvanKB/
git log --oneline -15 -- Tools/JarvanKB/
```

Expected:
- `git status` clean for `Tools/JarvanKB/` (only the gstack pre-existing M from before)
...
```bash
cd ~/Codes/awesome_agent_tools/
git add Tools/JarvanKB/docs/sendbox/toOrchestrator/
