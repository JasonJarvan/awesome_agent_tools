...
| SP-2 | 知乎引擎 | ⚫ done | sp2impler | 完成 2026-06-02（merge `f8c14cb`，51 tests + 真站 smoke 全过；纯 cookie+HTTP 无签名/无浏览器）；Step 8 RepoMem.merge 已完成（impler-driven HITL，提升知乎链路根因/坑到 `crawl-pipeline.md`）；契约 `Engine/zhihu/docs/interface.md`；**v1.1 评论完整树 ⚫ done**（merge `9081cbc`，58 tests + live smoke；Step 8 闭环：decisions.md D7 + 提升 child_comment offset 坑到 `crawl-pipeline.md` §知乎链路，by zhihucommentimpler）；**v1.2 专栏/ARTICLE api-fallback 🔴 blocked**（root g4 直属子 ZhihuArticleImpler；**§4 live-probe 推翻前提**：`/api/v4/articles/{id}` 需 `x-zse-96` 签名（403 code 10003，与 D5 的无签名 `/api/v4/answers/{id}` 不同）→ 镜像 ANSWER 兜底**不可行**，未自行引签名器；且 20/20 专栏 nav-GET 200、引擎原路径全文可抓 → 28 专栏失败实为 **bulk 瞬时 403**（限流）非结构性门。已 escalate → `toOrchestrator/from-zhihuarticleimpler-blocker-zse96.md`，待 root 决策 **UN-028**（A 引擎有界重试/退避 守 D1 / B 引签名器 破 D1 root 专属 / C 不改引擎归 watcher 限速）；证据 `Engine/zhihu/docs/RepoMem/temp/zhihu-engine-article-fallback/probe-findings.md`） |
...
| UN-028 | A | **决定 SP-2 v1.2 方向（blocker）**：live-probe 证 `/api/v4/articles/{id}` 需 `x-zse-96` 签名（403 code 10003）→ 镜像 ANSWER 无签名兜底**不可行**；且专栏 nav-GET 实测 200、28 失败为 bulk 瞬时限流非结构性门。三选一：**A** 引擎加 nav-GET 403 有界重试/退避（守 D1 无签名，惠及全类型）/ **B** 引 `x-zse-96` 签名器（破 D1，root 专属决策，证据不足不建议）/ **C** 不改引擎、归 watcher 限速 + seen-id 跨轮重试。impler 倾向 A，并请定重试落点（引擎 vs watcher） | `docs/sendbox/toOrchestrator/from-zhihuarticleimpler-blocker-zse96.md` | SP-2 v1.2 方向（专栏 bulk 可抓） | 2026-06-07 | open |
