...
SP-0 ✓ ──┬─ SP-1 ✓ ──┬─ SP-3(知乎Skill)   ⚫ done ← SP-2 ✓
         ...
         │            ├─ SP-5a(知乎Watcher)⚫ done(v1.1✓;分类SP已greenlit)
...
| UN-034 | B | **起 GrillDocsImpler session**（新会话，cwd=`Tools/JarvanKB/`），第一句：`read docs/sendbox/toGrillDocsImpler/handoff.md and start`。**装社区原版 grill-with-docs 替换当前适配版（设计评审门→CONTEXT.md 生成器）+ 调和被它引用的治理（CLAUDE.md/longterm 的 #4 prop4 设计门，brainstorm 定丢弃 or 换名保留）+ 在项目根跑出 `CONTEXT.md`**。root g4 直属子，完报 `toOrchestrator/`。注：此举**部分推翻上一轮 CodeTeam #4 prop4**（user 已知并裁决）；首个 gate=定位社区原版真源，不确定则问你 | `docs/sendbox/toGrillDocsImpler/handoff.md` | grill-with-docs 语义替换 + 项目 CONTEXT.md | 2026-06-10 | open |
| UN-033 | F | **(可选,不阻塞) 跑 SP-4b live 真站 smoke**:补 `MIMO_API_KEY`(impl 会话里为空)+ 让引擎 `config/bilibili-engine.yaml` 在 skill CWD 可解析(真件在 `Engine/bilibili/config/`)+ `bilibili-crawl` `config.yaml`(cookie 可选,缺则降级公开视频 ASR),跑 `python Skill/crawl/bilibili-crawl/scripts/live_smoke.py "<公开BV>"` 验显式+vague 两路。离线 29 测试已过,此为真站确认 | `Skill/crawl/bilibili-crawl/scripts/live_smoke.py` + `docs/sendbox/toBilibiliCrawlOrche/from-sp4bimpler-sp4b-done.md` | (无 — 可选真站验证,不阻塞 SP-6) | 2026-06-10 | open |
...
| UN-035 | Ops | **(垂直级,不阻塞 SP-5b 代码) 修 BN 下载器 HTTP 412**:SP-5b live smoke 实测 BN 的 yt-dlp 取 bilibili 元数据持续 `HTTP 412 Precondition Failed`(bilibili 对 BN 容器下载器风控)→ 转写成功路径(落 markdown)跑不通。**推 cookie 给 BN(`/api/update_downloader_cookie`)实测不解**。**整个 B 站垂直共担**(SP-4b live smoke 同样会撞)。候选修法(BN/ops 侧,非 SP-5b 代码):更新 BN 的 yt-dlp / 换 BN 容器出口 IP / 补 wbi+反检测 headers。SP-4a 2026-06-02 曾成功 → 风控近期收紧。**SP-5b 代码已完成+验证**(44 测试 + live 管线 + §5 失败降级 live 实证),BN 恢复后下轮自动重试落盘。另:**引擎 `BiliNoteClient` 未设 `trust_env=False`**→继承宿主 SOCKS `ALL_PROXY` 致连本地 BN 报 socksio ImportError;部署须清 `ALL_PROXY`(已记 runbook) | `Service/crawl/bilibili-watcher/docs/RepoMem/temp/sp5b-bilibili-watcher/smoke-evidence.md` + 信件 `docs/sendbox/toBilibiliCrawlOrche/from-sp5bimpler-blocker-bn412-downloader.md` | B站垂直 live 转写(SP-5b/SP-4b) | 2026-06-10 | open |
