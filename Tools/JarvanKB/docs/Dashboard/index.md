...
| UN-027 | B | **起 ZhihuArticleImpler session**（新会话，cwd=`Tools/JarvanKB/`），第一句：`read docs/sendbox/toZhihuArticleImpler/handoff.md and start SP-2 v1.2`。SP-2 引擎扩展=专栏/ARTICLE 403 加 `/api/v4` 无签名兜底(镜像 ANSWER 路径);跨 SP 修一处惠及 SP-3+SP-5a;**root g4 直属子**(非 SubOrche),完报 `from-zhihuarticleimpler-done.md` 回 `toOrchestrator/`;源自 ZhihuWatcherV11Impler 的 finding，root 接管 | `docs/sendbox/toZhihuArticleImpler/handoff.md` | SP-2 v1.2 落地（专栏可爬） | 2026-06-07 | open |
