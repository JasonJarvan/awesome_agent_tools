...
# Grill Design (JarvanKB v2 design-gate)

Deep design review of a DRAFT spec, grounded in the repository's durable knowledge. Adapted from the community grill-with-docs (ADR/CONTEXT.md-based) to recipe v2 (CodeTeam #4 prop 4, instantiated 2026-06-10; renamed grill-with-docs → grill-design 2026-06-11 to free the name for the community original — plan `grill-with-docs-restore` D1): the doc context here is RepoMem, NOT docs/adr/ or CONTEXT.md — do not create those.
...
- Auto-judge tier: run when there is a non-trivial design worth grilling (new architecture, new contract surface, cross-module behavior). Skip mechanically-full tasks (e.g. dependency bumps) with a one-line note in the plan: `grill-design: skipped — <reason>`.
